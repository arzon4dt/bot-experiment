...
function ItemPurchaseThink()
	...
	if  supportExist ~= nil and supportExist and hero_roles[npcBot:GetUnitName()]['support'] == 1 then
...
function IsSupportExist()
	local bot = GetBot();
	if hero_roles[bot:GetUnitName()]['support'] == 1 then
		return true;
	end
	...
	do
		...
		if hero_roles[ally:GetUnitName()]['support'] == 1	 
		then
			return true;
		end
	end
...
function GeneralItemPurchasing()
	...
	--[[if string.find(npcBot:GetUnitName(), "phantom_assassin") then
		print(sNextItem);
	end]]--
	...
	if( GetItemStockCount( "item_courier" ) > 0 and DotaTime() < 0.0  ) then
		if(npcBot:DistanceFromFountain() <= 600 ) then
			if ( npcBot:ActionImmediate_PurchaseItem("item_courier") == PURCHASE_ITEM_SUCCESS ) then
				--print(npcBot:GetUnitName().." purchase courier")
...
function PurchaseWard()
	...
	--if (minute % 5 == 0 or DotaTime() < 0) then
		...
		then
			--print(npcBot:GetUnitName().." purchase wards")
